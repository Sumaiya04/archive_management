...
Class UserDAO{
        ...
        if (!empty($_FILES['image']['tmp_name'])) {
            ...
            if ($uploaded == 0) {
                ...
                if(isset($_POST['update'])){
                    $del_dir=$_POST['imagesrc'];
                    unlink($del_dir);
                }
                if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_dir.$User->getID().'.'.$fileType)) {
                    $Upload = array();
                    //returns a boolean value
                    $Upload['uploaded'] = $uploaded;
                    //returns the directory of the uploaded file
                    $Upload['target_file'] = $target_dir.$User->getID().'.'.$fileType;
                    return $Upload;
                } else {
            ...
            if (!isset($_POST['update'])){
                ...
                $Upload['uploaded']=1;
                $Upload['target_file']=$_POST['imagesrc'];
                return $Upload;
            }
        }
    }
	...
	public function updateUserDetails($UserDetails){
		...
		$SQL = "UPDATE tbl_user_details SET Image='".$UserDetails->getImage()."', FatherName='".$UserDetails->getFatherName()."', 
